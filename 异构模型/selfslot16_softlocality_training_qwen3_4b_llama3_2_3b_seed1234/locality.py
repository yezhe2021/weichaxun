...
def forward(cfg, family, model, module, row, split):
    ...
    key, value = module(payload["k"].cuda(), payload["v"].cuda())
    logits = final_logits(model, row["encoded"][family]["suffix"], key, value)
    return logits, payload["native_logits"].cuda(), key, value


def loss(cfg, family, model, module, row, split):
    student, teacher, _, _ = forward(cfg, family, model, module, row, split)
...
def evaluate(cfg, family, model, module, selected, split):
    ...
    names = ("full_kl", "choice_kl", "native_agreement", "accuracy", "slot_k_cosine", "slot_v_cosine",
             ...
             "mean_attended_normalized_distance")
    totals = {name: 0.0 for name in names}; lengths = []
    for row in selected:
        student, teacher, key, value = forward(cfg, family, model, module, row, split)
        ids = row["encoded"][family]["choice_ids"]
        prediction, native_prediction = int(student[ids].argmax()), int(teacher[ids].argmax())
        entropy, effective, maximum, distance = attention_stats(module, native(cfg, family, split, row)["k"])
...
def train_one(cfg, family, strength, model, initial_queries):
    ...
    summary = {
        ...
        "same_query_initialization_across_strengths": True, "gold_used_for_training": False,
...
def run(cfg):
    ...
    save_json(run_root(cfg) / "results/summary.json", {
        "experiment": "16_slot_soft_locality_increased_training",
        "controlled": "16 ordered slots, lambda=16, 1024 train examples, four true epochs, 512 optimizer steps and 4096 sample exposures",
        "locality_formula": "-lambda * abs((t+0.5)/T - (i+0.5)/16)",
        "results": result,
    })
    log("ALL 16-SLOT INCREASED-TRAINING TESTS COMPLETED")
