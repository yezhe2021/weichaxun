...
def post_rope(model, k):
    positions = torch.arange(k.shape[1], device=k.device).unsqueeze(0)
...
def final_logits(model, ids, k=None, v=None, official=None):
    if official is not None and k is not None: raise ValueError('Ambiguous cache')
    cache = make_cache(model, k, v) if k is not None else copy.deepcopy(official)
    prefix = int(cache.get_seq_length()) if cache is not None else 0
    input_ids = torch.tensor([ids], device='cuda', dtype=torch.long)
    output = model.model(
        input_ids=input_ids,
        attention_mask=torch.ones(1, prefix + len(ids), dtype=torch.long, device='cuda'),
        position_ids=torch.arange(prefix, prefix + len(ids), device='cuda').unsqueeze(0),
