...
def main():
    ...
    except BlockingIOError: raise RuntimeError("Native-KV Translator pipeline already running")
    (root / "pipeline.pid").write_text(str(os.getpid()) + "\n")
    save_json(root / "run_config.json", cfg)
    save_json(root / "status.json", {"status": "running", "stage": "nativekv_translator", "pid": os.getpid()})
    try:
        run_all(cfg)
        save_json(root / "status.json", {"status": "completed", "stage": "nativekv_translator", "pid": os.getpid()})
    except BaseException as error:
        save_json(root / "status.json", {"status": "failed", "stage": "nativekv_translator",
