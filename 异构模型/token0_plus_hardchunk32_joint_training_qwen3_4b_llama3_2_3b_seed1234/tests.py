...
def run_tests():
    ...
    # Short sequences keep all 32 cache positions, but empty chunks emit zero KV
    # and are marked invalid for the receiver prefix attention mask.
    short_module = HardChunkSlotCompressor(2, slots=32, heads=2, dim=4)
    short_key, short_value = torch.randn(2, 20, 2, 4), torch.randn(2, 20, 2, 4)
    short_weights = short_module.attention(short_key)
    valid = short_module.valid_slots(20, short_key.device)
    assert valid.sum().item() == 20 and (~valid).sum().item() == 12
    assert torch.count_nonzero(short_weights[..., ~valid, :]) == 0
    short_k, short_v = short_module(short_key, short_value)
    assert short_k.shape == short_v.shape == (2, 32, 2, 4)
    assert torch.count_nonzero(short_k[:, ~valid]) == 0
    assert torch.count_nonzero(short_v[:, ~valid]) == 0
    ...
    return {"passed": True, "checks": ["balanced_contiguous_chunks", "complete_partition",
            ...
            "32_slot_short_sequence", "empty_chunk_zero_kv", "empty_chunk_receiver_mask",
