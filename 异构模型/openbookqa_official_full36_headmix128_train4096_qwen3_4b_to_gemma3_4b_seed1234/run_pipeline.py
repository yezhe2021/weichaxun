...
STAGES = (
    "model_audit", "prepare", "cache_sender_qwen", "cache_receiver_gemma", "phase0_audit", "teachers", "baselines",
    "stage_a", "select_stage_a", "train_residual", "train_shared", "evaluate",
)


def execute(cfg, stage):
    if stage == "model_audit":
        from model_audit import run
        run(cfg)
    elif stage == "prepare":
        from data import prepare_manifests
        prepare_manifests(cfg)
    elif stage == "cache_sender_qwen":
        from data import cache_sender_qwen
        cache_sender_qwen(cfg)
    elif stage == "cache_receiver_gemma":
        from data import cache_receiver_gemma_and_pairs
        cache_receiver_gemma_and_pairs(cfg)
        ...
        from evaluate_baselines import run
        run(cfg)
    else:
...
def run_stage(cfg, stage):
    ...
    try:
        ...
        save_json(run_root(cfg) / "status.json", {
            "status": "failed", "stage": stage, "error": f"{type(error).__name__}: {error}", "pid": os.getpid()
        })
        traceback.print_exc()
        raise
...
def main():
    ...
    try:
        ...
        raise RuntimeError("Reverse train4096 pipeline already running")
    ...
    print("ALL REVERSE TRAIN4096 EXPERIMENT STAGES COMPLETED", flush=True)
