...
class NativeKVTranslator(nn.Module):
    """Full36 Qwen3-4B [36,T,8,128] -> Gemma3-4B [34,T,4,256]."""

    def __init__(self, architecture="full36_headmix128", source_layers=36, target_layers=34,
                 source_heads=8, target_heads=4, source_dim=128, target_dim=256,
                 hidden_dim=1024, depth_output_dim=128, head_mapping="full_head"):
        super().__init__()
        if architecture != "full36_headmix128":
            raise ValueError(architecture)
        if head_mapping not in ("fixed_merge", "block_head", "full_head"):
        ...
        if head_mapping == "full_head":
            ...
            # Each target head receives one fixed adjacent pair of Qwen source heads.
            self.k_head = nn.ModuleList([nn.ModuleList([
                nn.Linear(2 * depth_output_dim, target_dim, bias=False)
                for _ in range(target_heads)]) for _ in range(target_layers)])
    ...
    def _depth(self, tensor, mappings):
        # [B,36,T,8,128] -> [B,T,8,4608] -> [B,34,T,8,128]
    ...
    def _heads(self, tensor, mappings):
        ...
        for layer in range(self.target_layers):
            ...
            if self.head_mapping == "block_head":
                current = torch.stack([
                    mappings[layer][head](current[:, :, 2 * head:2 * head + 2].flatten(-2))
                    for head in range(self.target_heads)
                ], dim=2)
            else:
                current = mappings[layer](current.flatten(-2)).reshape(
                    current.shape[0], current.shape[1], self.target_heads, self.target_dim)
            outputs.append(current)
        return torch.stack(outputs, dim=1)

    def forward(self, key, value):
        expected = (self.source_layers, 32, self.source_heads, self.source_dim)
...
class ResidualKVAdapter(nn.Module):
    """Independent Gemma receiver-space residuals for every layer/head and K/V."""

    def __init__(self, layers=34, heads=4, dim=256, rank=64):
        super().__init__()
