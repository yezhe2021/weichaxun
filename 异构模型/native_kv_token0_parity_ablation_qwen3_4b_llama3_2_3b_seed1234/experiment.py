...
def condition_indices(length, condition):
    if condition == "keep_even":
        return stride_indices(length, 0)
    if condition == "keep_odd":
        return stride_indices(length, 1)
    if condition == "keep_odd_plus_token0":
        odd = stride_indices(length, 1)
        return torch.cat((torch.zeros(1, dtype=torch.long), odd))
    if condition == "keep_even_minus_token0":
        return torch.arange(2, length, 2, dtype=torch.long)
...
def evaluate_family(cfg, family, rows, signature, output_root):
    ...
    conditions = {"native_full": [], **{name: [] for name in condition_names}}
        ...
        try:
            for number, row in enumerate(rows, 1):
                ...
                for name in condition_names:
                    indices = condition_indices(original_tokens, name)
...
def main():
    ...
    save_json(output_root / "summary.json", {
        "experiment": "native_kv_token0_parity_ablation",
        "question": "Does token0 explain the functional gap between even and odd stride-2 Native KV?",
        "training": "none",
        "results": results,
    })
    save_json(output_root / "status.json", {"status": "completed", "samples": samples})
    log("ALL NATIVE KV TOKEN0 PARITY ABLATIONS COMPLETED")
