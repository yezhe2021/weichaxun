...
from pairs import load_pair, pair_source_cfg, rows
...
def old_teacher_path(cfg, split, row):
    return Path(cfg["pair_source_root"]) / "teacher_cache" / split / f"{row['id']}.pt"


def load_teacher(cfg, split, row):
    payload = torch.load(old_teacher_path(cfg, split, row), map_location="cpu", weights_only=True)
    if payload["signature"] != pair_source_cfg(cfg)["signature"]:
        raise RuntimeError("Reused teacher cache signature mismatch")
    return payload
