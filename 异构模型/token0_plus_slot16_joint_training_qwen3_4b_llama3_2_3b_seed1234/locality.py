...
def run(cfg):
    ...
    save_json(run_root(cfg) / "results/summary.json", {
        "experiment": "token0_plus_slot16_joint_training",
        "controlled": "fixed Native token0 bypass plus 16 learned slots over tokens 1..T-1; 1024 train examples, four true epochs, 512 optimizer steps and 4096 exposures",
        "locality_formula": "-lambda * abs((t+0.5)/T - (i+0.5)/16)",
        "position_protocol": "token0=0, slots=1..16, receiver suffix starts at 17",
        "results": result,
    })
    log("ALL TOKEN0 PLUS 16-SLOT JOINT TRAINING COMPLETED")
