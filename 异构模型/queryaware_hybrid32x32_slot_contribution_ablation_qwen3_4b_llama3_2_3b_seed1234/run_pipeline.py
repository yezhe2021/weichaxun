...
def main():
    ...
    except BlockingIOError: raise RuntimeError("slot-contribution ablation already running")
    (root / "pipeline.pid").write_text(str(os.getpid()) + "\n")
    save_json(root / "run_config.json", cfg)
    save_json(root / "status.json", {"status": "running", "stage": "slot_contribution_ablation", "pid": os.getpid()})
    try:
        run(cfg)
        save_json(root / "status.json", {"status": "completed", "stage": "slot_contribution_ablation", "pid": os.getpid()})
    except BaseException as error:
        save_json(root / "status.json", {"status": "failed", "stage": "slot_contribution_ablation",
