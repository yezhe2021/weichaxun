...
def run_tests():
    ...
    wide_slot = SoftLocalSlotCompressor(2, slots=64, heads=2, dim=4, locality_strength=16)
    wide_k, wide_v = wide_slot(key[:, 1:], value[:, 1:])
    wide_joint_k = torch.cat((key[:, :1], wide_k), dim=1)
    wide_joint_v = torch.cat((value[:, :1], wide_v), dim=1)
    assert wide_joint_k.shape == wide_joint_v.shape == (2, 65, 2, 4)
    ...
    return {"passed": True, "checks": ["shape", "query_gradient", "Writer(0)=0",
            "native_token0_bypass", "64_slot_joint_shape", "kv_synchronized",
