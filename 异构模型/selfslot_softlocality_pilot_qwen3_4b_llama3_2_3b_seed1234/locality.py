...
def run(cfg):
    ...
    save_json(run_root(cfg) / "results/summary.json", {
        "experiment": "64_slot_global_vs_soft_locality_quick_pilot",
        "controlled": "same 64 slots, examples, objective, LR, 128 optimizer steps, 1024 exposures and identical query initialization",
        "locality_formula": "-lambda * abs((t+0.5)/T - (i+0.5)/64)",
        "results": result,
    })
    log("ALL SOFT-LOCALITY TESTS COMPLETED")
