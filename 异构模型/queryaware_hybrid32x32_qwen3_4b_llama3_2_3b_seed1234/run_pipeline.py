...
def main():
    ...
    except BlockingIOError: raise RuntimeError("query-aware hybrid pipeline is already running")
    (root / "pipeline.pid").write_text(str(os.getpid()) + "\n")
    save_json(root / "run_config.json", cfg)
    save_json(root / "status.json", {"status": "running", "stage": "queryaware_hybrid32x32", "pid": os.getpid()})
    try:
        run(cfg)
        save_json(root / "status.json", {"status": "completed", "stage": "queryaware_hybrid32x32", "pid": os.getpid()})
    except BaseException as error:
        save_json(root / "status.json", {"status": "failed", "stage": "queryaware_hybrid32x32",
