...
class HardChunkSlotCompressor(nn.Module):
    ...
    def __init__(self, layers, slots=16, heads=8, dim=128):
        super().__init__()
    ...
    def chunk_bounds(self, tokens):
        if tokens < self.slots:
            raise ValueError(f"Hard chunking needs at least {self.slots} tokens, observed {tokens}")
    ...
    def attention(self, key):
        ...
        mask = self.chunk_mask(key.shape[1], key.device)[None, None]
