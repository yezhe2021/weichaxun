...
def attention_stats(module, native_key):
    ...
    maximum = torch.where(sizes > 1, sizes.log(), torch.ones_like(sizes))
    ...
    return ((entropy / maximum[None, None]).mean().item(), entropy.exp().mean().item(),
...
def run(cfg):
    ...
    save_json(run_root(cfg) / "results/summary.json", {
        "experiment": "token0_plus_hardchunk16_joint_training",
        "controlled": "only locality changes versus token0 plus soft-local16: each learned slot attends exclusively within one balanced contiguous chunk",
        "locality_formula": "C_i=[floor(iN/16),floor((i+1)N/16)); A_i=softmax(q_i K_Ci^T/sqrt(d))",
        "position_protocol": "token0=0, slots=1..16, receiver suffix starts at 17",
        "results": result,
    })
    log("ALL TOKEN0 PLUS HARD-CHUNK16 JOINT TRAINING COMPLETED")
