...
from functional_probes import HiddenTokenProbe, KVFunctionalProbe
...
def build_probe(stage, examples, latent_dim, classes):
    feature = examples[0]["feature"]
    if torch.is_tensor(feature):
        return HiddenTokenProbe(feature.shape[-1], latent_dim, classes)
    return KVFunctionalProbe(
        len(feature["keys"]), feature["keys"][0].shape[-1], latent_dim, classes
    )


def train_probe(model, examples, labels, args, device):
...
def main():
    ...
    for stage in stages:
        ...
        torch.manual_seed(args.seed)
        model = build_probe(stage, train, args.latent_dim, len(labels)).to(device)
        history = train_probe(model, train, labels, args, device)
        ...
        torch.save({"model": model.state_dict(), "stage": stage, "args": vars(args)}, stage_out / "probe.pt")
