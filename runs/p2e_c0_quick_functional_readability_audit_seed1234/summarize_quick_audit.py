...
def probe_row(path, branch, stage, representation):
    ...
    return {
        ...
        "metric": "trained_functional_probe",
    }
...
def main():
    ...
    with open(output / "SUCCESS.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "status": "complete",
                "experiment": "P2-E-C0 quick segmented functional readability audit",
                "table": table,
                "branches": branches,
                "claim_boundary": (
                    "This 64/16/16 quick audit identifies directional drops only. "
                    "It is not a statistical-significance replacement for the full audit."
                ),
            },
            handle,
            indent=2,
