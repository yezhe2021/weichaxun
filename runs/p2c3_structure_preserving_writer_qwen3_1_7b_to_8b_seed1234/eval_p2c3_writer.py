...
from p2c3_writer import StructurePreservingNativeKVWriter, shape_only_memory
from train_p2c3_writer import _resample_token_axis, value_mismatched_memory
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate one P2-C3 structure-preserving Writer variant")
    ...
    route_rows = []
    structure_rows = []
    for pair_index in tqdm(range(pair_count), desc=f"p2c3_{writer_args['variant']}_free_running"):
        ...
        conditions = [
            ...
            ("enhanced_writer_1_7b_base", student_prefixed_prompt(tokenizer, base), writer_base, base["answer"], base["answer"], True, "accuracy"),
            ("enhanced_writer_1_7b_counterfactual", student_prefixed_prompt(tokenizer, base), writer_cf, counterfactual["answer"], counterfactual["answer"], True, "accuracy"),
        ...
        for variant, row, writer_memory, native_memory in (
            ...
            structure = structure_preservation_losses(
                ...
                native_diag,
                aligned,
                int(writer_args["relation_max_tokens"]),
            )
            structure_rows.append(
                {
                    "pair_id": base["pair_id"],
                    "memory_variant": variant,
                    **detached_structure_metrics(structure),
                }
            )
    ...
    writer_pc = paired_consistency(records, "enhanced_writer_1_7b_base", "enhanced_writer_1_7b_counterfactual")
    ...
    writer_em = source_em(conditions, "enhanced_writer_1_7b_base", "enhanced_writer_1_7b_counterfactual")
    raw_em = source_em(conditions, "raw_minimal_1_7b_base", "raw_minimal_1_7b_counterfactual")
    paired_gap = paired_gap_bootstrap(
        records,
        "enhanced_writer_1_7b_base",
        "enhanced_writer_1_7b_counterfactual",
    ...
    summary = {
        ...
        "paired_consistency": {
            "full_text": text_pc,
            "native_8b": native_pc,
            "enhanced_writer_1_7b": writer_pc,
        ...
        "structure_summary": {
            ...
            "token_alignment_rate": float(np.mean([row["token_aligned"] for row in structure_rows])),
        },
