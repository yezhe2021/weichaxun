...
def shape_only_memory(memory, receiver_layers, receiver_heads, receiver_head_dim, output_dtype=None):
    ...
    for receiver_layer in range(receiver_layers):
        ...
        if sender_heads <= receiver_heads:
            head_indices = [
            ...
            value_groups = []
            for head in range(receiver_heads):
                start = int(math.floor(head * sender_heads / receiver_heads))
                end = int(math.floor((head + 1) * sender_heads / receiver_heads))
                end = max(start + 1, end)
                key_groups.append(layer_keys[start:end].mean(dim=0))
                value_groups.append(layer_values[start:end].mean(dim=0))
            layer_keys = torch.stack(key_groups, dim=0)
            layer_values = torch.stack(value_groups, dim=0)
