...
def main():
    ...
    actions = [("selftest", None), ("audit", None), ("extract", None)]
    actions += [("stage_a", variant) for variant in cfg["train_variants"]]
    actions += [("stage_b", variant) for variant in cfg["train_variants"]]
    ...
    for action, variant in actions:
        ...
        argv = [
            ...
            "--config", str(CONFIG), "--mode", args.mode, action,
        ]
        if variant:
