...
def main():
    ...
    for pair_index in tqdm(range(pair_count), desc="p2b_free_running"):
        ...
        conditions = [
            ("full_text_base", full_text_prefixed_prompt(tokenizer, base), None, base["answer"], False, "accuracy"),
            ("full_text_counterfactual", full_text_prefixed_prompt(tokenizer, counterfactual), None, counterfactual["answer"], False, "accuracy"),
            ("native_8b_base", student_prefixed_prompt(tokenizer, base), native_base, base["answer"], True, "accuracy"),
            ("native_8b_counterfactual", student_prefixed_prompt(tokenizer, base), native_cf, counterfactual["answer"], True, "accuracy"),
            ("writer_4b_base", student_prefixed_prompt(tokenizer, base), writer_base, base["answer"], True, "accuracy"),
            ("writer_4b_counterfactual", student_prefixed_prompt(tokenizer, base), writer_cf, counterfactual["answer"], True, "accuracy"),
            ("raw_4b_base", student_prefixed_prompt(tokenizer, base), raw_base, base["answer"], True, "accuracy"),
            ("raw_4b_counterfactual", student_prefixed_prompt(tokenizer, base), raw_cf, counterfactual["answer"], True, "accuracy"),
            ("writer_shuffled", student_prefixed_prompt(tokenizer, base), writer_other, base["answer"], True, "original_answer_leakage"),
            ("writer_mismatched", student_prefixed_prompt(tokenizer, base), mismatched_memory(writer_base, writer_other), base["answer"], True, "original_answer_leakage"),
            ("zero_kv", student_prefixed_prompt(tokenizer, base), zero_memory(writer_base), base["answer"], True, "original_answer_leakage"),
            ("reader_off", student_prefixed_prompt(tokenizer, base), writer_base, base["answer"], False, "original_answer_leakage"),
        ]
    ...
    writer_pc = paired_consistency(records, "writer_4b_base", "writer_4b_counterfactual")
    raw_pc = paired_consistency(records, "raw_4b_base", "raw_4b_counterfactual")
    native_em = source_em(conditions, "native_8b_base", "native_8b_counterfactual")
    writer_em = source_em(conditions, "writer_4b_base", "writer_4b_counterfactual")
    raw_em = source_em(conditions, "raw_4b_base", "raw_4b_counterfactual")
    summary = {
        "status": "complete",
        "args": vars(args),
        "paired_consistency": {
        ...
        "native_gap_recovery": {
            ...
            "paired_consistency_writer_vs_raw": recovery(writer_pc, raw_pc, native_pc),
        },
