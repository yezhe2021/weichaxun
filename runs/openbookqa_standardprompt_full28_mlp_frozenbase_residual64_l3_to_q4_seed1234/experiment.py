...
from translator import NativeKVTranslator, ResidualKVAdapter, component_loss


def base_run(cfg):
    return Path(cfg["base_experiment"]) / "runs" / "study"


def stage_a_checkpoint(cfg):
    selection = read_json(base_run(cfg) / "stage_a" / "selection.json")
...
def load_base(cfg, frozen=True):
    ...
    old_cfg = read_json(base_run(cfg) / "run_config.json")
    if payload.get("signature") != old_cfg["signature"] or payload.get("architecture") != "full28_mlp":
...
def prepare_oracle_teachers(cfg, model):
    for split in ("train", "validation", "test"):
        ...
        for number, row in enumerate(selected, 1):
            ...
            save_tensor(teacher_path(cfg, split, row), {"signature": cfg["signature"], "choice_logits": choice,
