    ...
    def __init__(self, model, max_gate=0.5, gate_init=0.0, query_rank=0, output_rank=0):
        ...
        if self.query_rank > 0:
            self.query_down = nn.ModuleList(
                [nn.Linear(hidden_size, self.query_rank, bias=False) for _ in range(self.layer_count)]
            )
            self.query_up = nn.ModuleList(
                [nn.Linear(self.query_rank, hidden_size, bias=False) for _ in range(self.layer_count)]
            )
