...
ROOT="/home/yezhe/伪查询/runs/p3e_g_strong_reader_v1_seed1234"
...
if [[ ! -f "$ROOT/smoke16/TRAIN_SUCCESS.json" ]]; then
  "$PY" "$ROOT/train_strong_reader.py" \
    --model "$MODEL4" \
...
if [[ ! -f "$ROOT/eval64/SUCCESS.json" ]]; then
  "$PY" "$ROOT/eval_strong_reader.py" \
    --model "$MODEL4" \
    --memory-index "$VAL_CACHE" \
    --data "$P3D3/data/validation.jsonl" \
    --base-reader "$C1" \
    --strong-reader "$ROOT/formal512/checkpoint_best.pt" \
    --out "$ROOT/eval64" \
    --seed 1234 \
    --device cuda
fi
...
"$PY" "$ROOT/audit.py" --root "$ROOT" --base-reader "$C1"
