...
def main():
    ...
    args = parser.parse_args(); result = read_json(args.result); correct = result["conditions"]["correct_learnable_duplicate16"]["f1"]
    ...
    write_json(args.out, {"status": "complete", "passed": passed,
        "thresholds": {"correct_f1": args.min_correct_f1, "correct_shuffled_gap": args.min_gap, "reader_off_consistency": args.min_off_consistency},
        "observed": {"correct_f1": correct, "correct_shuffled_gap": gap, "reader_off_consistency": off},
        "action": "run_formal512" if passed else "stop_before_formal512"})
