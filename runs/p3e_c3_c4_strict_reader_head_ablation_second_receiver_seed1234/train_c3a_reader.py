...
def main():
    ...
    parser.add_argument("--model", required=True); parser.add_argument("--memory", required=True); parser.add_argument("--writer", required=True); parser.add_argument("--out", required=True)
    parser.add_argument("--init", choices=["fully_random", "weak_pair"], required=True); parser.add_argument("--seed", type=int, required=True)
    parser.add_argument("--max-samples", type=int, default=512); parser.add_argument("--epochs", type=int, default=20); parser.add_argument("--lr", type=float, default=2e-4); parser.add_argument("--weight-decay", type=float, default=0.01)
    parser.add_argument("--depend-weight", type=float, default=0.5); parser.add_argument("--margin", type=float, default=0.5); parser.add_argument("--rank", type=int, default=32); parser.add_argument("--top-k", type=int, default=2); parser.add_argument("--gate-init", type=float, default=0.01)
    parser.add_argument("--temperature-start", type=float, default=1.0); parser.add_argument("--temperature-end", type=float, default=0.25); parser.add_argument("--max-length", type=int, default=512); parser.add_argument("--max-new-tokens", type=int, default=32)
    ...
    model, tokenizer = load_receiver(args.model, device); model.requires_grad_(False); model.eval(); writer, _ = load_writer(args.writer, device); writer.requires_grad_(False); writer.eval()
    selected_layers = list(cache.inner.index["original_layer_indices"])
    ...
    initialization = initialize_fresh_reader(reader, args.init, args.seed, args.gate_init)
    optimizer = torch.optim.AdamW(reader.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    expected = {id(parameter) for parameter in reader.parameters()}; actual = {id(parameter) for group in optimizer.param_groups for parameter in group["params"]}
    if expected != actual or any(parameter.requires_grad for parameter in model.parameters()) or any(parameter.requires_grad for parameter in writer.parameters()): raise RuntimeError("C3-A trainable boundary failed")
    monitor_indices = random.Random(args.seed + 388).sample(indices, min(args.monitor_samples, total)); history, best_loss, best_epoch = [], float("inf"), -1
    for epoch in range(1, args.epochs + 1):
        ...
        for sample_index in tqdm(order, desc=f"c3a_{args.init}_seed{args.seed}_epoch{epoch}"):
            ...
            if any(parameter.grad is not None for parameter in model.parameters()) or any(parameter.grad is not None for parameter in writer.parameters()): raise RuntimeError("Gradient escaped C3-A Reader")
        ...
        if epoch % args.monitor_every == 0 or epoch == args.epochs:
            metrics, rows = monitor(model, tokenizer, reader, writer, cache, negatives, monitor_indices, device, args.max_new_tokens); record["train_free_running_monitor"] = metrics
        ...
        if epoch_loss < best_loss:
            ...
            torch.save({"reader": state, "reader_metadata": reader.metadata(), "initialization": initialization, "epoch": epoch, "train_loss": best_loss, "args": vars(args),
                        "writer": args.writer, "writer_sha256": file_sha256(args.writer), "writer_frozen": True, "receiver_backbone_frozen": True,
                        "native_reader_loaded": False, "c1_reader_loaded": False}, output / "checkpoint_best.pt")
    write_json(output / "TRAIN_SUCCESS.json", {"status": "complete", "experiment": "C3-A strict fresh Reader", "initialization": initialization, "samples": total,
        "best_epoch_by_train_objective": best_epoch, "best_train_loss": best_loss, "writer_frozen": True, "checkpoint": str(output / "checkpoint_best.pt")})
