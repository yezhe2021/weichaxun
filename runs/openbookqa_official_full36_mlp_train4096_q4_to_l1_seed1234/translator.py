...
class NativeKVTranslator(nn.Module):
    """Qwen4B [36,T,8,128] -> Llama1B [16,T,8,64], without token/head mixing."""

    def __init__(self, architecture="full36_mlp_q4_to_l1", source_layers=36, target_layers=16,
                 heads=8, source_dim=128, target_dim=64, hidden_dim=1024):
        super().__init__()
        if architecture != "full36_mlp_q4_to_l1":
            raise ValueError(architecture)
        self.architecture = architecture
        self.source_layers, self.target_layers = source_layers, target_layers
        self.heads, self.source_dim, self.target_dim = heads, source_dim, target_dim
        ...
        self.k_head = nn.ModuleList([
            nn.ModuleList([nn.Linear(target_dim, target_dim, bias=False) for _ in range(heads)])
    ...
    def forward(self, key, value):
        expected = (self.source_layers, 32, self.heads, self.source_dim)
...
class ResidualKVAdapter(nn.Module):
    """Independent receiver-space adapters for every Llama1B target layer/head and K/V."""

    def __init__(self, layers=16, heads=8, dim=64, rank=64):
        super().__init__()
