        ...
        for split in ("train", "validation", "test"):
            ...
            for number, row in enumerate(selected_rows, 1):
                ...
                if number % 32 == 0 or number == len(selected_rows):
                    log(f"Qwen source decision cache {split}: {number}/{len(selected_rows)}")
    finally:
        del model; torch.cuda.empty_cache()


@torch.no_grad()
def cache_llama_and_pairs(cfg):
    toks = {family: tokenizer(path) for family, path in cfg["models"].items()}
        ...
        for split in ("train", "validation", "test"):
            ...
            for number, row in enumerate(selected_rows, 1):
                ...
                option_spans = {family: [span for span in spans[family]
                    ...
                    for family in ("qwen", "llama")}
                anchors = build_anchors(row["body"], option_spans["qwen"], option_spans["llama"],
                ...
                self_anchors = build_anchors(row["body"], option_spans["llama"], option_spans["llama"],
                ...
                payload = {"signature": cfg["signature"], "id": row["id"],
                    ...
                    "qwen_full_choice_logits": source["full_choice_logits"],
                    "llama_full_choice_logits": choice_logits,
                    "metadata": {"anchors": anchors, "source_indices": [0] + source_indices,
                                 "target_indices": target_indices, "llama_self_indices": self_indices,
                                 "llama_self_anchors": self_anchors,
                                 "llama_selected_attention_mass": selected_mass(
                                     importance, self_indices[1:], fields["option_token_indices"]),
                                 "llama_body_tokens": len(fields["body"]), "llama_full_tokens": len(fields["full"]),
                                 "llama_answer_token_indices": fields["answer_token_indices"],
                                 "llama_decision_query_index": fields["decision_query_index"]}}
                if split == "test":
                    payload["llama_self_k"] = key[:, self_indices[1:]].contiguous()
                    payload["llama_self_v"] = value[:, self_indices[1:]].contiguous()
                save_tensor(path, payload)
                if number % 32 == 0 or number == len(selected_rows):
                    log(f"Llama target decision/pair cache {split}: {number}/{len(selected_rows)}")
    finally:
        del model; torch.cuda.empty_cache()
...
def phase0_audit(cfg):
    ...
    for split, row in chosen:
        ...
        record = {"sample_id": row["id"], "split": split, "raw_prompt": row["full_prompt"],
                  ...
                  "source_qwen_answer_token_indices": row["encoded"]["qwen"]["answer_token_indices"],
                  "target_llama_answer_token_indices": row["encoded"]["llama"]["answer_token_indices"],
        ...
        if any(index in record["source_qwen_answer_token_indices"] for index in record["selected_32_indices"]):
