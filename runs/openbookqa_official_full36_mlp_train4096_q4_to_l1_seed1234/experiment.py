...
def load_base(cfg, frozen=True):
    ...
    if payload.get("signature") != cfg["signature"] or payload.get("architecture") != "full36_mlp_q4_to_l1":
        raise RuntimeError("Stage-A checkpoint protocol mismatch")
    module = NativeKVTranslator("full36_mlp_q4_to_l1", hidden_dim=cfg["mlp_hidden_dim"]).cuda()
...
def save_stage_a(cfg, module, step, validation):
    ...
    save_tensor(path, {"signature": cfg["signature"], "architecture": "full36_mlp_q4_to_l1", "step": step,
...
def load_stage_a_candidate(cfg, path):
    ...
    module = NativeKVTranslator("full36_mlp_q4_to_l1", hidden_dim=cfg["mlp_hidden_dim"]).cuda().eval()
    module.load_state_dict(payload["state"], strict=True)
...
def train_stage_a(cfg):
    seed_all(cfg["seed"])
    module = NativeKVTranslator("full36_mlp_q4_to_l1", hidden_dim=cfg["mlp_hidden_dim"]).cuda().train()
...
def load_branch(cfg, kind):
    ...
    if kind == "shared":
        base = NativeKVTranslator("full36_mlp_q4_to_l1", hidden_dim=cfg["mlp_hidden_dim"]).cuda().eval()
...
def evaluate(cfg, model):
    ...
    if baseline_path.exists():
        ...
        oracle_accuracy = result["baselines"]["metrics"]["llama1_native_oracle32"]["accuracy"]
        result["translation_gap"] = {
            "definition": "Llama1 Native Oracle32 accuracy minus translated accuracy",
