...
mkdir -p "$ROOT/teacher_cache" "$ROOT/smoke16" "$ROOT/formal512" \
  "$ROOT/eval64" "$ROOT/review" "$ROOT/audit"

if [[ ! -f "$ROOT/teacher_cache/SUCCESS.json" ]]; then
  "$PY" "$ROOT/cache_text_teacher.py" \
    --model "$MODEL4" \
    --data "$P3D3/data/train.jsonl" \
    --out "$ROOT/teacher_cache" \
    --max-samples 512 \
    --max-length 4096 \
    --top-k 256 \
    --device cuda
...
if [[ ! -f "$ROOT/smoke16/TRAIN_SUCCESS.json" ]]; then
  "$PY" "$ROOT/train_assimilation_reader.py" \
    --model "$MODEL4" \
    --memory-index "$TRAIN_CACHE" \
    --data "$P3D3/data/train.jsonl" \
    --negatives "$P3EF/data/hard_negatives.json" \
    --teacher-index "$ROOT/teacher_cache/index.json" \
    --base-reader "$C1" \
    --out "$ROOT/smoke16" \
    --mode smoke16 \
    --max-samples 16 \
...
if [[ ! -f "$ROOT/eval64/SUCCESS.json" ]]; then
  "$PY" "$ROOT/eval_assimilation_reader.py" \
    --model "$MODEL4" \
    --memory-index "$VAL_CACHE" \
    --data "$P3D3/data/validation.jsonl" \
    --base-reader "$C1" \
    --assimilation-reader "$ROOT/formal512/checkpoint_best.pt" \
    --out "$ROOT/eval64" \
    --seed 1234 \
    --device cuda
fi
