...
from p2e_structure import detached_metrics, span_relation_losses, transport_alignment_losses
...
def verify_cache_prefix(sender_cache, teacher_cache):
    if len(sender_cache) > len(teacher_cache):
        raise ValueError("Sender cache is longer than teacher cache")
...
def main():
    parser = argparse.ArgumentParser(description="Train Qwen3.5 full-attention KV to Qwen3-8B Writer")
