...
from p2a_common import load_jsonl, parse_dtype, resolve_device, sender_text
...
def qwen35_sender_prompt(tokenizer, row):
    ...
    if marker < 0:
        raise ValueError("The calibrated Qwen3.5 prompt lost its NOW SOLVE marker")
...
def text_backbone(model):
    ...
    if backbone is None or not hasattr(backbone, "layers"):
        raise RuntimeError("Qwen3.5 text backbone model.language_model.layers was not found")
    return backbone
...
def full_attention_layer_indices(model):
    ...
    if not indices:
        raise RuntimeError("Qwen3.5 contains no full-attention layers")
    return indices


@torch.inference_mode()
def encode_example(model, tokenizer, qwen_tokenizer, row, teacher_example, device, max_length):
    prompt, prompt_spans = qwen35_sender_prompt(tokenizer, row)
    encoded = tokenizer(
        prompt,
        return_tensors="pt",
    ...
    source_layer_indices = full_attention_layer_indices(model)
    captured_keys = [None] * len(source_layer_indices)
    captured_values = [None] * len(source_layer_indices)
    handles = []
    for cache_index, layer_index in enumerate(source_layer_indices):
        layer = backbone.layers[layer_index]
        attention = layer.self_attn

        def hook(module, args, kwargs, cache_index=cache_index):
            hidden = kwargs.get("hidden_states", args[0] if args else None)
            key, value = project_native_kv(module, hidden)
            captured_keys[cache_index] = key[0, :, indices, :].detach().half().cpu()
            captured_values[cache_index] = value[0, :, indices, :].detach().half().cpu()
    ...
    if any(tensor is None for tensor in captured_keys + captured_values):
        raise RuntimeError(f"Failed to capture every Qwen3.5 full-attention layer for {row['id']}")
    if not output.hidden_states:
        raise RuntimeError("Qwen3.5 did not return text hidden states")
    final_hidden = output.hidden_states[-1][0, indices, :].detach().half().cpu()
    return {
        **row,
        "memory": {
            "keys": captured_keys,
        ...
        "final_hidden_tokens": final_hidden,
        ...
        "prompt_style": "qwen35_fewshot_join_reason",
    }


def main():
    parser = argparse.ArgumentParser(description="Cache Qwen3.5 full-attention Native KV")
    parser.add_argument(
        "--model",
        default="/home/yezhe/all_models/models/Qwen/Qwen3___5-4B",
    )
    ...
    if len(pairs) > len(teacher_cache.entries):
        raise ValueError("Teacher cache is shorter than requested Qwen3.5 cache")
    ...
    sender_counts = []
    teacher_counts = []
    for pair_index, (pair_id, variants) in enumerate(tqdm(pairs, desc="qwen35_native_kv_pairs")):
    ...
    with open(output / "index.json", "w", encoding="utf-8") as handle:
        json.dump(
            {
                "format_version": 4,
                "coordinate_system": "qwen35_full_attention_pre_rope_k_native_v_with_character_transport",
                "model": args.model,
                "teacher_index": args.teacher_index,
                "data": args.data,
                "prompt_style": "qwen35_fewshot_join_reason",
                "pairs": len(pair_files),
                "layers": len(source_layer_indices),
