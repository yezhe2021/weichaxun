...
def load_base(cfg, frozen=True):
    ...
    if payload.get("signature") != cfg["signature"] or payload.get("architecture") != "full16_mlp":
        raise RuntimeError("Stage-A checkpoint protocol mismatch")
    module = NativeKVTranslator("full16_mlp", hidden_dim=cfg["mlp_hidden_dim"]).cuda()
...
def save_stage_a(cfg, module, step, validation):
    ...
    save_tensor(path, {"signature": cfg["signature"], "architecture": "full16_mlp", "step": step,
...
def load_stage_a_candidate(cfg, path):
    ...
    module = NativeKVTranslator("full16_mlp", hidden_dim=cfg["mlp_hidden_dim"]).cuda().eval()
    module.load_state_dict(payload["state"], strict=True)
...
def train_stage_a(cfg):
    seed_all(cfg["seed"])
    module = NativeKVTranslator("full16_mlp", hidden_dim=cfg["mlp_hidden_dim"]).cuda().train()
...
def load_branch(cfg, kind):
    ...
    if kind == "shared":
        base = NativeKVTranslator("full16_mlp", hidden_dim=cfg["mlp_hidden_dim"]).cuda().eval()
