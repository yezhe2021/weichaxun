...
def run(cfg):
    ...
    try:
        for number, row in enumerate(rows, 1):
            ...
            if not torch.allclose(captured.float(), source["full_choice_logits"].float(), atol=2e-3, rtol=2e-3):
                raise RuntimeError(f"Llama1B full-native cache drift: {row['id']}")
            ...
            logits = final_logits(llama, fields["receiver_answer"], key, value,
                                  positions=torch.arange(key.shape[1], device="cuda"), suffix_start=key.shape[1])
            llama_selected[row["id"]] = logits[fields["choice_ids"]].cpu()
            if number % 32 == 0 or number == len(rows):
                print(f"Llama1B baseline pass: {number}/{len(rows)}", flush=True)
    finally:
        del llama
    ...
    try:
        for number, row in enumerate(rows, 1):
            ...
            choices = {
                ...
                "llama1_full_native": source["full_choice_logits"],
                "llama1_native_selected32": llama_selected[row["id"]],
    ...
    key_pairs = (
        ...
        ("llama1_full_native", "llama1_native_selected32"),
    ...
    summary = {
        ...
        "definitions": {
            "llama1_native_selected32": "Llama1B-native Question + its selected 32 native Option KVs + native Answer.",
            "qwen_native_oracle32": "Qwen-native Question + Qwen-native KVs at Llama1B-selected RawAnchor positions + native Answer.",
