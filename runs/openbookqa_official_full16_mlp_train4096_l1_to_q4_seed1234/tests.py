...
if __name__ == "__main__":
    ...
    assert sum(p.numel() for p in module.parameters())==9437184
    translator=NativeKVTranslator("full16_mlp")
    source_k=torch.randn(1,16,32,8,64); source_v=torch.randn_like(source_k)
    target_k,target_v=translator(source_k,source_v)
    assert target_k.shape==(1,36,32,8,128) and target_v.shape==target_k.shape
    assert all(layer.bias is None for layer in translator.modules() if isinstance(layer,torch.nn.Linear))
    assert sum(p.numel() for p in translator.parameters())==94371840
    print("Full16-MLP geometry and Residual64 identity/zero/bias tests passed",flush=True)
