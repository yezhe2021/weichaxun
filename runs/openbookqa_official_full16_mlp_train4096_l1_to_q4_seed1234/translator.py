...
class NativeKVTranslator(nn.Module):
    """Full16 MLP: Llama1B [16,T,8,64] -> Qwen4B [36,T,8,128]."""

    def __init__(self, architecture="full16_mlp", source_layers=16, target_layers=36,
                 heads=8, source_dim=64, target_dim=128, hidden_dim=1024):
        super().__init__()
        if architecture != "full16_mlp":
...
class ResidualKVAdapter(nn.Module):
    """Independent receiver-space adapters for every Qwen target layer/head and K/V."""
