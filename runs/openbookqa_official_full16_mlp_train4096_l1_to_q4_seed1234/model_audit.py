...
def run(cfg):
    expected = {"llama": (16, 8, 64), "qwen": (36, 8, 128)}
    records = {}
    for family, path in cfg["models"].items():
        ...
        records[family] = {
            "path": path,
            "layers": observed[0], "kv_heads": observed[1], "head_dim": observed[2],
            "tokenizer_class": type(tok).__name__, "bos_token_id": tok.bos_token_id,
        }
    records["translator"] = {
        "source_shape": [16, 32, 8, 64], "target_shape": [36, 32, 8, 128],
        "per_head_depth_width": 1024, "hidden_dim": cfg["mlp_hidden_dim"],
