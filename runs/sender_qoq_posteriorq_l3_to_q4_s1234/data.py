...
def serialize(row):
    ...
    posterior_question = question_header + question
    sender_prompt = body + posterior_question
    ...
    return {"body": body, "question_prefix": question_prefix,
            ...
            "full_prompt": full_prompt, "posterior_question": posterior_question,
...
def encode(tok, serialized):
    ...
    sender_plain = enc(serialized["sender_prompt"])
    ...
    if sender_plain[:len(body_plain)] != body_plain:
        raise RuntimeError("Noncompositional body/posterior-Question tokenizer boundary")
    ...
    body_ids, full_ids, sender_ids = bos + body_plain, bos + full_plain, bos + sender_plain
    choice_ids = []
    ...
    return {"body": body_ids, "answer": answer_ids, "receiver_answer": receiver_answer_ids,
            ...
            "sender_full": sender_ids,
            "answer_token_indices": list(range(len(body_ids), len(full_ids))),
            "decision_query_index": len(sender_ids) - 1, "choice_ids": choice_ids}
...
def prepare_manifests(cfg):
    ...
    summary = {"signature": cfg["signature"],
               "sender_prompt": "Question -> Options -> repeated Question",
               "receiver_prompt": "native Question -> external selected Options KV -> native Answer:",
...
def baseline_pair(cfg, split, row):
    root = Path(cfg["baseline_experiment_root"]) / "runs" / "study"
    baseline_cfg = read_json(root / "run_config.json")
    if baseline_cfg.get("protocol") != "receiver_native_question_options_only_v1":
        raise RuntimeError("Unexpected standard-prompt baseline protocol")
    payload = torch.load(root / "pair_cache" / split / f"{row['id']}.pt",
                         map_location="cpu", weights_only=True)
    if payload.get("signature") != baseline_cfg["signature"]:
        raise RuntimeError("Standard-prompt baseline pair mismatch")
    return payload
    ...
    try:
        for split in ("train", "validation", "test"):
            ...
            for number, row in enumerate(selected_rows, 1):
                ...
                key, value, importance, _ = capture_decision(
                    model, fields["sender_full"], len(fields["body"]), fields["choice_ids"])
                choice_logits = baseline_pair(cfg, split, row)["llama_full_choice_logits"]
                ...
                save_tensor(path, {"signature": cfg["signature"], "id": row["id"],
                    ...
                    "metadata": {"anchors": anchors, "selected_indices": indices[1:],
                                 ...
                                 "body_tokens": len(fields["body"]), "sender_tokens": len(fields["sender_full"]),
    ...
    try:
        for split in ("train", "validation", "test"):
            ...
            for number, row in enumerate(selected_rows, 1):
                ...
                key, value, importance, _ = capture_decision(
                    model, fields["sender_full"], len(fields["body"]), fields["choice_ids"])
                baseline = baseline_pair(cfg, split, row)
                choice_logits = baseline["qwen_full_choice_logits"]
                ...
                payload = {"signature": cfg["signature"], "id": row["id"],
                    ...
                    "llama_full_choice_logits": baseline["llama_full_choice_logits"],
                    "metadata": {"anchors": anchors, "source_indices": [0] + source_indices,
                                 ...
                                 "qwen_body_tokens": len(fields["body"]),
                                 "qwen_sender_tokens": len(fields["sender_full"]),
...
def phase0_audit(cfg):
    ...
    for split, row in chosen:
        ...
        record = {"sample_id": row["id"], "split": split,
                  "sender_prompt": row["sender_prompt"], "receiver_standard_prompt": row["full_prompt"],
    ...
    summary = {"passed": True, "sample_count": len(records),
               ...
               "query": "last token of the repeated posterior Sender Question",
               "candidates": "Options only; both Question copies, position0 and Answer excluded",
