...
from p2c1_writer import HeterogeneousNativeKVWriter
...
class LazyPairCache:
    def __init__(self, index_path, capacity=2):
        ...
        if self.index.get("format_version") not in (3, 4):
            raise ValueError("Expected a format_version=3 or 4 pair cache")
...
def main():
    parser = argparse.ArgumentParser(description="Train a Qwen3-1.7B Writer for a frozen Qwen3-8B Query-only Reader")
    ...
    parser.add_argument("--layer-width", type=int, default=5)
    ...
    writer_parameter_ids = {id(parameter) for parameter in writer.parameters()}
    optimizer_parameter_ids = {
        id(parameter)
        for group in optimizer.param_groups
        for parameter in group["params"]
    }
    if optimizer_parameter_ids != writer_parameter_ids:
        raise RuntimeError("Optimizer must contain all and only Writer parameters")
    ...
    if any(parameter.requires_grad for parameter in reader.parameters()):
        raise RuntimeError("Query-only Reader is not fully frozen")
    ...
    for epoch in range(args.epochs):
        ...
        progress = tqdm(order, desc=f"p2c1_writer_epoch_{epoch}")
