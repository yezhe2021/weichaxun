...
from p2e_structure import (
    identity_token_transport,
    project_route_to_teacher,
)
from p2e_writer import StructurePreservingNativeKVWriter, shape_only_memory
from train_p2g2_writer import _resample_token_axis, value_mismatched_memory
...
def key_mismatched_memory(unrelated, positive):
    keys = [
        _resample_token_axis(key, value.shape[1])
        for key, value in zip(unrelated["keys"], positive["values"])
    ]
    output = {"keys": keys, "values": positive["values"]}
...
def route_readout_rows(pair_id, variant, writer_diag, native_diag, transport):
    rows = []
    for layer in sorted(int(key) for key in writer_diag if key.isdigit()):
        ...
        writer_route = project_route_to_teacher(
            writer_slot["route_tensor"].float(), transport["sender_mass_to_teacher"]
        )
        ...
        rows.append(
            {
                ...
                "layer": layer,
                "token_aligned": True,
                "alignment": "identical_qwen_tokenizer_evidence_tokens",
                "route_kl": route_kl,
...
def prediction_switch_rate(records, left, right):
    left_rows = {row["pair_id"]: row for row in records if row["condition"] == left}
    right_rows = {row["pair_id"]: row for row in records if row["condition"] == right}
    common = left_rows.keys() & right_rows.keys()
    return float(np.mean([left_rows[key]["prediction"] != right_rows[key]["prediction"] for key in common]))
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate one P2-G2 Qwen3-8B to Qwen3-4B Writer")
    ...
    if sender_cache.index.get("model") != "/home/yezhe/all_models/models/Qwen/Qwen3-8B":
        raise ValueError(f"Unexpected sender cache model: {sender_cache.index.get('model')!r}")
    if teacher_cache.index.get("model") != args.receiver_model:
        raise ValueError(f"Teacher cache model {teacher_cache.index.get('model')!r} != receiver")
    ...
    if reader_args.get("model") != args.receiver_model:
        raise ValueError("Reader checkpoint does not belong to the Qwen3-4B receiver")
    if reader_args.get("config_name") != "query_only" or int(reader_args.get("query_rank", -1)) != 32:
        raise ValueError("P2-G2 requires the frozen rank-32 Query-only Reader")
    ...
    if writer_args.get("receiver_model") != args.receiver_model:
        raise ValueError("Writer checkpoint targets a different receiver model")
    sender_geometry = writer_checkpoint["sender_geometry"]
    receiver_geometry = writer_checkpoint["receiver_geometry"]
    observed_sender = {key: int(sender_cache.index[key]) for key in ("layers", "kv_heads", "head_dim")}
    observed_receiver = {key: int(teacher_cache.index[key]) for key in ("layers", "kv_heads", "head_dim")}
    if sender_geometry != observed_sender or receiver_geometry != observed_receiver:
        raise ValueError("Writer checkpoint geometry does not match the evaluation caches")
    features = writer_checkpoint["features"]
    writer = StructurePreservingNativeKVWriter(
        ...
        top_k=int(writer_args["top_k"]),
        adapter_rank=int(writer_args["adapter_rank"]),
        shared_routing=bool(features["shared_routing"]),
        route_residual_scale=float(writer_args["route_residual_scale"]),
        teacher_k_rms=writer_checkpoint["writer"]["teacher_k_rms"],
    ).to(device).eval()
    ...
    layer_rows = []
    route_rows = []
    for pair_index in tqdm(range(pair_count), desc=f"p2e_{writer_args['variant']}_free_running"):
        ...
        conditions = [
            ...
            ("native_4b_base", student_prefixed_prompt(tokenizer, base), native_base, base["answer"], base["answer"], True, "accuracy"),
            ("native_4b_counterfactual", student_prefixed_prompt(tokenizer, base), native_cf, counterfactual["answer"], counterfactual["answer"], True, "accuracy"),
            ("writer_8b_to_4b_base", student_prefixed_prompt(tokenizer, base), writer_base, base["answer"], base["answer"], True, "accuracy"),
            ("writer_8b_to_4b_counterfactual", student_prefixed_prompt(tokenizer, base), writer_cf, counterfactual["answer"], counterfactual["answer"], True, "accuracy"),
            ("raw_minimal_8b_base", student_prefixed_prompt(tokenizer, base), raw_base, base["answer"], base["answer"], True, "accuracy"),
            ("raw_minimal_8b_counterfactual", student_prefixed_prompt(tokenizer, base), raw_cf, counterfactual["answer"], counterfactual["answer"], True, "accuracy"),
            ...
            ("writer_value_mismatched", student_prefixed_prompt(tokenizer, base), value_mismatched_memory(writer_base, writer_other), base["answer"], other_sender_pair["base"]["answer"], True, "original_answer_leakage"),
            ("writer_key_mismatched", student_prefixed_prompt(tokenizer, base), key_mismatched_memory(writer_other, writer_base), base["answer"], base["answer"], True, "key_route_ablation"),
        ...
        for variant, row, writer_memory, native_memory in (
            ...
            teacher_row = teacher_pair[variant]
            transport = identity_token_transport(row, teacher_row)
            route_rows.extend(
                route_readout_rows(
                    base["pair_id"],
                    variant,
    ...
    native_pc = paired_consistency(records, "native_4b_base", "native_4b_counterfactual")
    writer_pc = paired_consistency(records, "writer_8b_to_4b_base", "writer_8b_to_4b_counterfactual")
    raw_pc = paired_consistency(records, "raw_minimal_8b_base", "raw_minimal_8b_counterfactual")
    native_em = source_em(conditions, "native_4b_base", "native_4b_counterfactual")
    writer_em = source_em(conditions, "writer_8b_to_4b_base", "writer_8b_to_4b_counterfactual")
    raw_em = source_em(conditions, "raw_minimal_8b_base", "raw_minimal_8b_counterfactual")
    paired_gap = paired_gap_bootstrap(
        records,
        "writer_8b_to_4b_base",
        "writer_8b_to_4b_counterfactual",
        "raw_minimal_8b_base",
        "raw_minimal_8b_counterfactual",
    )
    ...
    summary = {
        "status": "complete",
        "variant": writer_args["variant"],
        "args": vars(args),
        "paired_consistency": {
            "full_text": text_pc,
            "native_4b": native_pc,
            "writer_8b_to_4b": writer_pc,
            "raw_minimal_8b": raw_pc,
        },
        "prediction_switch_rate": {
            "full_text": prediction_switch_rate(records, "full_text_base", "full_text_counterfactual"),
            "native_4b": prediction_switch_rate(records, "native_4b_base", "native_4b_counterfactual"),
            "writer_8b_to_4b": prediction_switch_rate(records, "writer_8b_to_4b_base", "writer_8b_to_4b_counterfactual"),
            "raw_minimal_8b": prediction_switch_rate(records, "raw_minimal_8b_base", "raw_minimal_8b_counterfactual"),
        },
        ...
        "route_readout_summary": {
            "route_kl_mean": float(np.mean([row["route_kl"] for row in route_rows])),
        ...
        "token_alignment": "strict_identity",
        "kv_routing_difference": {
            key: float(value.detach().cpu())
            for key, value in writer.routing_difference_tensors().items()
        },
    }
    ...
    write_jsonl(output / "global_layer_routing.jsonl", writer.routing_diagnostics())
