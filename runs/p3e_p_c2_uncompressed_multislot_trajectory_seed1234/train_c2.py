...
from p3e_p_c2_common import C2TrajectoryReader
...
def main():
    ...
    reader = C2TrajectoryReader(model, SELECTED_LAYERS).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, index in enumerate(
            tqdm(order, desc=f"p3e_p_c2_epoch{epoch}")
        ):
            ...
            if packed["answer_token_ids"] != teacher["answer_token_ids"]:
                raise RuntimeError("Teacher/student token mismatch")
            trace = {}
            optimizer.zero_grad(set_to_none=True)
            with reader.inject(
                model,
                native_memory(payload, device),
            ...
            teacher_states = teacher["teacher_states"].float().to(device)
            state_loss = normalized_state_loss(corrected, teacher_states)
            total_loss = answer_loss + 0.5 * state_loss
            total_loss.backward()
    ...
    write_json(
        output / "TRAIN_SUCCESS.json",
        {
            ...
            "loss": "answer + 0.5 * normalized_state_mse",
            "slots": 8,
            "slot_dim": 256,
            "slot_pooling_before_state_query": False,
            "receiver_frozen": True,
            "only_c2_reader_updated": True,
        },
    )
