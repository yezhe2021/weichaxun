...
def load_checkpoint(cfg, path):
    ...
    module = NativeKVTranslator(payload["architecture"], hidden_dim=cfg["mlp_hidden_dim"]).cuda()
...
def train_stage_a(cfg):
    seed_all(cfg["seed"]); module = NativeKVTranslator(
        cfg["architecture"], hidden_dim=cfg["mlp_hidden_dim"]).cuda()
...
def append_results_document(cfg, comparison):
    ...
    tag = "openbookqa_standardprompt_nativequestion_options32_full28_mlp_l3_to_q4_seed1234"
    ...
    section = "\n".join([begin, f"## {tag}", "",
        "Machine-generated after successful completion. Receiver-native Question + translated Options KV + Receiver-native Answer:.", "",
...
def finalize(cfg):
    ...
    comparison = {"experiment": "openbookqa_standardprompt_nativequestion_options32_full28_mlp_l3_to_q4_seed1234",
        "status": "completed", "prompt": "Question -> Options -> Answer:",
        "routing_query": "last Sender-native Answer: token", "selection": "32 option-only raw-text regions",
    ...
    log("OPENBOOKQA STANDARD-PROMPT FULL28-MLP EXPERIMENT COMPLETED")
    return comparison
