...
def old_rows(cfg, split):
    raw = read_json(cfg["dataset"])
    test_raw = random.Random(cfg["seed"]).sample(raw, 128)
    test_ids = {str(item["idx"]) for item in test_raw}
    remaining = [item for item in raw if str(item["idx"]) not in test_ids]
    random.Random(cfg["seed"] + 1).shuffle(remaining)
    partitions = {"train": remaining[:305], "validation": remaining[305:369], "test": test_raw}
    normalized = []
    for item in partitions[split]:
        ...
        if [value["label"] for value in choices] != list(LABELS):
            raise RuntimeError(f"Unexpected OpenBookQA labels: {item['idx']}")
        normalized.append({"id": str(item["idx"]), "category": "openbookqa_eng",
                           "question": item["question"]["stem"],
                           "options": [value["text"] for value in choices],
                           "gold_index": LABELS.index(item["answerKey"])})
    return normalized[:cfg[f"{split}_samples"]]
...
def prepare_manifests(cfg):
    ...
    summary = {"signature": cfg["signature"], "dataset": "AraDiCE OpenBookQA English test",
    ...
    if cfg["mode"] == "study" and sum(map(len, split_ids.values())) != 497:
        raise RuntimeError("OpenBookQA split does not cover all 497 examples")
