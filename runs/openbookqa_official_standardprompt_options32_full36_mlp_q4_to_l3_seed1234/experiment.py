...
def evaluate(cfg, model):
    modules = {kind: load_branch(cfg, kind) for kind in ("shared", "residual")}
    stage_a, stage_a_path, _ = load_base(cfg, frozen=True)
    selected = manifest_rows(cfg, "test")
    names = ("stage_a", "shared", "residual")
    ...
    result = {"status": "completed", "protocol": cfg["protocol"], "train_samples": cfg["train_samples"],
        ...
        "paired_shared_vs_residual": correctness_pair(correct["shared"], correct["residual"]),
        ...
        "trained_parameters": {kind: modules[kind][2]["parameters_trained"] for kind in ("shared", "residual")}}
