...
REUSED_HIDDEN_PROBE="${REUSED_HIDDEN_PROBE:-${P2EA}/probe_results/raw_evidence_attention_layer_28/checkpoint.pt}"
...
audit() {
  for path in \
    ...
    "${TASK_WRITER}" "${SHARED_WRITER}" "${REUSED_HIDDEN_PROBE}"; do require_file "${path}"; done
  "${PY}" -m py_compile \
...
manifest() {
  ...
  "${PY}" "${ROOT}/build_quick_manifest.py" \
    ...
    --out "${MANIFEST}" --train-pairs 448 --validation-pairs 64 --test-pairs 64 --seed "${SEED}"
}
...
memory_probes() {
  ...
  "${PY}" "${ROOT}/train_memory_probes.py" \
    ...
    --reused-hidden-checkpoint "${REUSED_HIDDEN_PROBE}" \
    --out "${MEMORY_RESULTS}" --epochs 30 --patience 6 --seed "${SEED}" --device "${DEVICE}"
}
...
chain_branch() {
  ...
  if [[ ! -f "${result}/CHAIN_STAGES_SUCCESS.json" ]]; then
    "${PY}" "${ROOT}/train_chain_probes.py" \
      ...
      --epochs 30 --patience 6 --seed "${SEED}" --device "${DEVICE}"
  fi
}
