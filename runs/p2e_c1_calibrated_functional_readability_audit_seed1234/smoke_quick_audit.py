...
def main():
    classes = 40
    check(ReusedAttentionPoolProbe(96, classes, 32, 64), torch.randn(17, 96), classes)
    memory = {
        "keys": [torch.randn(4, 19, 16) for _ in range(3)],
        "values": [torch.randn(4, 19, 16) for _ in range(3)],
    }
    check(KVFunctionalProbe(3, 4, 16, 32, 64, classes), memory, classes)
    ...
    print("P2-E-C1 synthetic probe smoke passed")
