...
class NativeKVExternalReader(nn.Module):
    def __init__(self, model, max_gate=0.5, gate_init=0.0, reader_rank=0):
        super().__init__()
        ...
        self.reader_rank = int(reader_rank)
        hidden_size = int(model.config.hidden_size)
        if self.reader_rank > 0:
            self.down = nn.ModuleList(
                [nn.Linear(hidden_size, self.reader_rank, bias=False) for _ in range(self.layer_count)]
            )
            self.up = nn.ModuleList(
                [nn.Linear(self.reader_rank, hidden_size, bias=False) for _ in range(self.layer_count)]
            )
