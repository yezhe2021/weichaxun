...
from common import log, read_json, run_root, save_json, save_tensor, seed_all
...
def stage_a_checkpoint(cfg):
    root = Path(cfg["stage_a_experiment"]) / "runs" / "study"
    selection = read_json(root / "stage_a" / "selection.json")
...
def load_base(cfg, frozen=True):
    ...
    source_cfg = read_json(Path(cfg["stage_a_experiment"]) / "runs" / "study" / "run_config.json")
    if payload.get("signature") != source_cfg["signature"] or payload.get("architecture") != "full28_mlp":
        raise RuntimeError("Stage-A checkpoint protocol mismatch")
    module = NativeKVTranslator("full28_mlp", hidden_dim=cfg["mlp_hidden_dim"]).cuda()
...
def save_stage_a(cfg, module, step, validation):
    ...
    save_tensor(path, {"signature": cfg["signature"], "architecture": "full28_mlp", "step": step,
...
def load_stage_a_candidate(cfg, path):
    payload = torch.load(path, map_location="cpu", weights_only=True)
    if payload.get("signature") != cfg["signature"] or payload.get("architecture") != "full28_mlp":
        raise RuntimeError("Fresh Stage-A checkpoint mismatch")
    module = NativeKVTranslator("full28_mlp", hidden_dim=cfg["mlp_hidden_dim"]).cuda().eval()
    module.load_state_dict(payload["state"], strict=True)
...
def evaluate(cfg, model):
    residual_base, residual_adapter, training = load_branch(cfg, "residual")
    stage_a, stage_a_path, _ = load_base(cfg, frozen=True)
    selected = manifest_rows(cfg, "test")
    names = ("stage_a", "residual128")
    ...
    for number, row in enumerate(selected, 1):
        ...
        outputs = {
            "stage_a": branch_output("shared", stage_a, None, source_item)[:2],
            "residual128": branch_output("residual", residual_base, residual_adapter, source_item)[:2],
        }
    ...
    rank64_root = Path(cfg["rank64_experiment"]) / "runs" / "study" / "results"
    rank64_result = read_json(rank64_root / "comparison.json")
    rank64_rows = [json.loads(line) for line in (rank64_root / "per_sample_metrics.jsonl").read_text(
        encoding="utf-8").splitlines() if line]
    rank64_map = {row["id"]: row for row in rank64_rows}
    rank64_correct = [bool(rank64_map[row["id"]]["conditions"]["residual"]["correct"]) for row in selected]
    for record in records:
        previous = rank64_map[record["id"]]["conditions"]["residual"]
        record["conditions"]["residual64"] = {
            "prediction": previous["prediction"], "correct": bool(previous["correct"]),
            "oracle_agreement": bool(previous["oracle_agreement"]),
        }
    result = {"status": "completed", "protocol": cfg["protocol"], "train_samples": cfg["train_samples"],
        ...
        "metrics": {**metrics, "residual64_reference": rank64_result["metrics"]["residual"]},
        "relative_residual_norm": {"k": quantiles(ratios["k"]), "v": quantiles(ratios["v"])},
        "paired_residual64_vs_residual128": correctness_pair(rank64_correct, correct["residual128"]),
        "paired_stage_a_vs_residual128": correctness_pair(correct["stage_a"], correct["residual128"]),
        "frozen_base_hash_verified": training["base_hash_before"] == training["base_hash_after"],
        "trained_parameters": {"residual128": training["parameters_trained"]},
        "stage_a_retrained_in_this_run": False, "shared_stage_b_run": False}
    ...
    del residual_base, residual_adapter, stage_a
    torch.cuda.empty_cache()
    return result
