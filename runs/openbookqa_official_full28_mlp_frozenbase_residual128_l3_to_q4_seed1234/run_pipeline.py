...
STAGES = ("asset_audit", "train_residual", "evaluate")
...
def execute(cfg, stage):
    if stage == "asset_audit":
        ...
        from experiment import evaluate, train_branch
        model = load_model(cfg, "qwen")
        try:
            if stage == "train_residual":
...
def main():
    ...
    try:
        ...
        raise RuntimeError("Residual128 pipeline already running")
    ...
    print("ALL RESIDUAL128 EXPERIMENT STAGES COMPLETED", flush=True)
