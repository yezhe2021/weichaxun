...
def main():
    parser = argparse.ArgumentParser(description="Evaluate one enhanced P2-C2 Writer variant")
    ...
    adapter_mode = "shared_full" if writer_args["variant"] == "global_only" else "per_head"
    ...
    for pair_index in tqdm(range(pair_count), desc="p2b_free_running"):
        ...
        conditions = [
            ...
            ("writer_value_mismatched", student_prefixed_prompt(tokenizer, base), mismatched_memory(writer_base, writer_other), base["answer"], other_sender_pair["base"]["answer"], True, "original_answer_leakage"),
            ("writer_key_mismatched", student_prefixed_prompt(tokenizer, base), mismatched_memory(writer_other, writer_base), base["answer"], base["answer"], True, "key_route_ablation"),
