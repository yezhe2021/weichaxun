...
def verify_cache_prefix(sender_cache, teacher_cache):
    if len(sender_cache) != len(teacher_cache):
        raise ValueError("Sender and teacher caches must contain the same number of pairs")
    sender_ids = [entry["pair_id"] for entry in sender_cache.entries]
    teacher_ids = [entry["pair_id"] for entry in teacher_cache.entries[: len(sender_cache)]]
    if sender_ids != teacher_ids:
        raise ValueError("Sender cache is not pair-aligned with the teacher prefix")
...
def variant_features(variant):
    return {
        ...
        "reader_alignment": variant == "reader_aligned",
    }


def build_reader(receiver, checkpoint_path, expected_model, device):
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    args = checkpoint["args"]
    if args.get("model") != expected_model:
        raise ValueError(f"Reader checkpoint model {args.get('model')!r} != receiver {expected_model!r}")
    if args.get("config_name") != "query_only" or int(args.get("query_rank", -1)) != 32:
...
def main():
    parser = argparse.ArgumentParser(description="Train the P2-G2 Qwen3-8B to frozen Qwen3-4B Reader Writer")
    ...
    parser.add_argument("--attention-mass-weight", type=float, default=0.005)
    parser.add_argument("--routing-binding-weight", type=float, default=0.01)
    parser.add_argument("--span-binding-weight", type=float, default=0.05)
    parser.add_argument("--span-relation-weight", type=float, default=0.025)
    ...
    verify_cache_prefix(sender_cache, teacher_cache)
    ...
    for parameter in receiver.parameters():
        parameter.requires_grad_(False)
    reader = build_reader(receiver, args.reader_checkpoint, args.receiver_model, device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        progress = tqdm(order, desc=f"p2e_{args.variant}_epoch_{epoch}")
        for position, pair_index in enumerate(progress):
            ...
            for variant in ("base", "counterfactual"):
                ...
                need_teacher = (
                    features["structure_loss"]
                    or features["reader_alignment"]
                    or use_auxiliary
                )
                ...
                attention_mass_loss = nll_target.new_zeros(())
                structure = {
                ...
                if need_teacher:
                    ...
                    route_loss, readout_loss = transport_alignment_losses(
                        writer_diag,
                        teacher_diag,
                        transport,
                        nll_target,
                    )
                    attention_mass_loss = target_mass_alignment(
                        writer_diag, teacher_diag, nll_target
                    )
                    if use_auxiliary or features["reader_alignment"]:
                        block = (
                            block
                            + args.route_weight * route_loss
                            + args.readout_weight * readout_loss
                            + args.attention_mass_weight * attention_mass_loss
                        )
                    if features["structure_loss"]:
                        ...
                        block = (
                            block
                            + args.span_binding_weight * structure["binding"]
                            + args.span_relation_weight
                ...
                row_metrics.update(
                    {
                        ...
                        f"{variant}_attention_mass_loss": float(
                            attention_mass_loss.detach().cpu()
                        ),
                        **{
                            f"{variant}_structure_{key}": value
                            for key, value in detached_metrics(structure).items()
                        },
                    }
                )
