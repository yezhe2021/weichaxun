...
class LazyPairCache:
    def __init__(self, index_path, capacity=2):
        ...
        if self.index.get("format_version") != 3:
            raise ValueError("P2-G1 requires format_version=3 pair cache")
        expected = {"layers": 36, "query_heads": 32, "kv_heads": 8, "head_dim": 128}
        observed = {name: self.index.get(name) for name in expected}
        if observed != expected:
            raise ValueError(f"Unexpected Qwen3-4B cache geometry: {observed}, expected {expected}")
...
def main():
    parser = argparse.ArgumentParser(description="Train the frozen-backbone Qwen3-4B Native Query Reader for P2-G1")
    ...
    parser.add_argument("--config-name", choices=("query_only",), default="query_only")
    parser.add_argument("--query-rank", type=int, default=32)
    parser.add_argument("--output-rank", type=int, default=0)
    ...
    expected_geometry = {"layers": 36, "query_heads": 32, "kv_heads": 8, "head_dim": 128}
    if geometry != expected_geometry:
        raise ValueError(f"Unexpected Qwen3-4B receiver geometry: {geometry}, expected {expected_geometry}")
    ...
    if not trainable or any(parameter.requires_grad for parameter in receiver.parameters()):
        raise RuntimeError("Freeze audit failed: only NativeKVExternalReader parameters may be trainable")
        ...
        for position, pair_index in enumerate(progress):
            ...
            if should_step:
                if any(parameter.grad is not None for parameter in receiver.parameters()):
                    raise RuntimeError("Frozen Qwen3-4B backbone received gradients")
        ...
        json.dump(
            {
                ...
                "freeze_audit": {
                    "receiver_trainable_parameters": 0,
                    "reader_trainable_parameters": sum(parameter.numel() for parameter in trainable),
                    "optimizer_matches_reader": True,
                },
