...
from train_p2e_writer import _resample_token_axis, value_mismatched_memory
        ...
        rows.append(
            {
                ...
                "layer": layer,
                "token_aligned": False,
                "alignment": "canonical_character_transport",
                "route_kl": route_kl,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate one Llama-3.2-3B to Qwen3-8B P2-E Writer")
    ...
    for pair_index in tqdm(range(pair_count), desc=f"p2e_{writer_args['variant']}_free_running"):
        ...
        conditions = [
            ...
            ("writer_llama3_2_3b_base", student_prefixed_prompt(tokenizer, base), writer_base, base["answer"], base["answer"], True, "accuracy"),
            ("writer_llama3_2_3b_counterfactual", student_prefixed_prompt(tokenizer, base), writer_cf, counterfactual["answer"], counterfactual["answer"], True, "accuracy"),
            ("raw_minimal_llama3_2_3b_base", student_prefixed_prompt(tokenizer, base), raw_base, base["answer"], base["answer"], True, "accuracy"),
            ("raw_minimal_llama3_2_3b_counterfactual", student_prefixed_prompt(tokenizer, base), raw_cf, counterfactual["answer"], counterfactual["answer"], True, "accuracy"),
        ...
        for variant, row, writer_memory, native_memory in (
            ...
            structure = span_relation_losses(
                writer_memory,
                native_memory,
                writer_diag,
                native_diag,
    ...
    writer_pc = paired_consistency(records, "writer_llama3_2_3b_base", "writer_llama3_2_3b_counterfactual")
    raw_pc = paired_consistency(records, "raw_minimal_llama3_2_3b_base", "raw_minimal_llama3_2_3b_counterfactual")
    native_em = source_em(conditions, "native_8b_base", "native_8b_counterfactual")
    writer_em = source_em(conditions, "writer_llama3_2_3b_base", "writer_llama3_2_3b_counterfactual")
    raw_em = source_em(conditions, "raw_minimal_llama3_2_3b_base", "raw_minimal_llama3_2_3b_counterfactual")
    paired_gap = paired_gap_bootstrap(
        records,
        "writer_llama3_2_3b_base",
        "writer_llama3_2_3b_counterfactual",
        "raw_minimal_llama3_2_3b_base",
        "raw_minimal_llama3_2_3b_counterfactual",
    )
    ...
    summary = {
        ...
        "paired_consistency": {
            "full_text": text_pc,
            "native_8b": native_pc,
            "writer_llama3_2_3b": writer_pc,
            "raw_minimal_llama3_2_3b": raw_pc,
        },
        ...
        "structure_summary": {
            "token_binding_error": float(np.mean([row["binding"] for row in structure_rows])),
            "key_relation_error": float(np.mean([row["key_relation"] for row in structure_rows])),
            "value_relation_error": float(np.mean([row["value_relation"] for row in structure_rows])),
            "readout_relation_error": float(np.mean([row["readout_relation"] for row in structure_rows])),
            "transport_coverage": float(np.mean([row["transport_coverage"] for row in structure_rows])),
        },
    ...
    write_jsonl(output / "per_sample_structure.jsonl", structure_rows)
