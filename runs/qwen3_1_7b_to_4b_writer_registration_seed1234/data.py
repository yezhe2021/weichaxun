...
class Store:
    ...
    def source(self, split, sample_id):
        path = Path(self.cfg["work_dir"]) / "cache" / "source1_7" / split / f"{sample_id}.pt"
...
def depth_interpolate(key, value, target_layers=36):
    """Fixed normalized-depth interpolation from 28 native layers to 36 target layers."""
    source_layers = key.shape[0]
    coordinates = torch.arange(target_layers, device=key.device, dtype=torch.float32) * ((source_layers - 1) / (target_layers - 1))
    lower = coordinates.floor().long(); upper = coordinates.ceil().long()
    alpha = (coordinates - lower.float()).to(key.dtype).view(target_layers, 1, 1, 1)
    return (
        key[lower] * (1 - alpha) + key[upper] * alpha,
        value[lower] * (1 - alpha) + value[upper] * alpha,
    )
