...
from data import (
    Store, capture_native, cuda, depth_interpolate, load_json, load_model,
...
def build_source(cfg,mode,rows):
    model=load_model(cfg["model_1_7b"],cfg)
    try:
        for split,samples in rows.items():
            ...
            directory=Path(cfg["work_dir"])/"cache"/"source1_7"/split; directory.mkdir(parents=True,exist_ok=True)
            for index,sample in enumerate(source_samples,1):
                ...
                progress(f"{mode}: source1_7 {split} {index}/{len(source_samples)}")
    finally:
...
def scales(cfg,mode,rows):
    ...
    for index,sample in enumerate(rows["train"],1):
        ...
        values={
            "source_k":depth_interpolate(source["pre_key"],source["value"],cfg["num_layers"])[0][:,positions].float().flatten(2),
            "source_v":depth_interpolate(source["pre_key"],source["value"],cfg["num_layers"])[1][:,positions].float().flatten(2),
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--config",required=True); parser.add_argument("--mode",choices=("smoke","development"),required=True); parser.add_argument("action",choices=("source","scales")); args=parser.parse_args()
