...
from data import (
    Store, cuda, depth_interpolate, load_json, load_model, normalize_answer, progress,
...
def sample_rep(cfg, writer, store, split, sample):
    ...
    sk, sv = depth_interpolate(source["pre_key"], source["value"], cfg["num_layers"])
    sk, sv = sk[:, positions].to(cuda()), sv[:, positions].to(cuda())
...
def train_stage_a(cfg, mode, kind, overfit=False, bootstrap=False):
    rows = rows_for(cfg, mode); store = Store(cfg, mode, rows)
    writer = (make_writer(kind, scales_for(cfg, mode), cfg).to(cuda()) if bootstrap else writer_for(cfg, mode, kind)).train()
    prefix = "depth_linear" if bootstrap else kind
    stage = f"{prefix}_{'overfit16' if overfit else 'stage_a'}"; out = root(cfg, mode, stage); out.mkdir(parents=True, exist_ok=True)
    if mode == "smoke": maximum = cfg["smoke_updates"]
    elif bootstrap: maximum = cfg["depth_overfit_updates"] if overfit else cfg["depth_stage_a_updates"]
    else: maximum = cfg["overfit_updates"] if overfit else cfg["stage_a_updates"]
    ...
    optimizer = (AdamW(writer.parameters(), lr=cfg["depth_stage_a_lr"], weight_decay=cfg["weight_decay"]) if bootstrap else optimizer_for(cfg, writer, "stage_a"))
    ...
    summary = {
        "kind": prefix, "overfit": overfit, "initialized_from": "identity" if bootstrap else str(old_linear_checkpoint(cfg)),
    ...
    if bootstrap and not overfit:
        reused = Path(cfg["work_dir"]) / "reused"; reused.mkdir(parents=True, exist_ok=True)
        destination = reused / "linear_stage_a_best.pt"; shutil.copy2(out / "best.pt", destination)
...
def full_source(writer, store, split, sample, source_id=None):
    ...
    key, value = depth_interpolate(record["pre_key"], record["value"], 36)
    return writer(key.to(cuda()), value.to(cuda()))
...
def train_stage_b(cfg, mode, kind, branch, initial_override=None, output_override=None):
    rows = rows_for(cfg, mode); store = Store(cfg, mode, rows)
    initial = Path(initial_override) if initial_override else root(cfg, mode, f"{kind}_stage_a") / "best.pt"
    writer = writer_for(cfg, mode, kind, initial).train(); model = load_model(cfg["model_4b"], cfg); tok = tokenizer(cfg["model_4b"])
    out = root(cfg, mode, output_override or f"{kind}_{branch}"); out.mkdir(parents=True, exist_ok=True)
...
class DepthControl(torch.nn.Module):
    def __init__(self, scales, use_rms):
        super().__init__(); self.use_rms = use_rms
        for name in ("source_k", "source_v", "target_k", "target_v"):
            self.register_buffer(name, scales[name].float())

    def forward(self, key, value):
        if not self.use_rms: return key, value
        shape = (key.shape[0], 1, 8, 128)
        return (
            key / self.source_k.view(shape).to(key.dtype) * self.target_k.view(shape).to(key.dtype),
            value / self.source_v.view(shape).to(value.dtype) * self.target_v.view(shape).to(value.dtype),
        )


def depth_baseline(cfg, mode):
    rows = rows_for(cfg, mode); store = Store(cfg, mode, rows); model = load_model(cfg["model_4b"], cfg); tok = tokenizer(cfg["model_4b"]); output = []
    scales = scales_for(cfg, mode)
    for name, use_rms in (("depth_raw", False), ("depth_rms", True)):
        control = DepthControl(scales, use_rms).to(cuda())
        output += evaluate_condition(cfg, model, tok, store, rows["test"], name, control)
    out = root(cfg, mode, "depth_baseline"); out.mkdir(parents=True, exist_ok=True)
    summary = aggregate(output)
    cache17 = load_json(Path(cfg["cache_audit_1_7b_dir"]) / "artifacts" / ("smoke" if mode == "smoke" else "development") / "summary.json")["conditions"]
    reference4 = load_json(Path(cfg["audit_4b_dir"]) / "artifacts" / "development" / "summary.json")["conditions"]
    summary["qwen3_1_7b_full_context_text"] = cache17["full_context_text"]
    for name in ("question_only", "full_context_text", "official_native_cache"):
        summary[f"qwen3_4b_{name}"] = reference4[name]
    save_json(out / "per_sample.json", output); save_json(out / "summary.json", summary)
    del model; torch.cuda.empty_cache()


def depth_linear_f0(cfg, mode):
    rows = rows_for(cfg, mode); store = Store(cfg, mode, rows); model = load_model(cfg["model_4b"], cfg); tok = tokenizer(cfg["model_4b"])
    checkpoint = root(cfg, mode, "depth_linear_stage_a") / "best.pt"; writer = writer_for(cfg, mode, "linear_continued", checkpoint)
    output = evaluate_condition(cfg, model, tok, store, rows["test"], "depth_linear_f0_correct", writer)
    output += evaluate_condition(cfg, model, tok, store, rows["test"], "depth_linear_f0_shuffled", writer, True)
    out = root(cfg, mode, "depth_linear_f0"); out.mkdir(parents=True, exist_ok=True); save_json(out / "per_sample.json", output); save_json(out / "summary.json", aggregate(output))
    del model, writer; torch.cuda.empty_cache()


def train_depth_stage_b(cfg, mode, branch):
    initial = root(cfg, mode, "depth_linear_stage_a") / "best.pt"
    train_stage_b(cfg, mode, "linear_continued", branch, initial, f"depth_linear_{branch}")


def depth_linear_evaluate(cfg, mode):
    rows = rows_for(cfg, mode); store = Store(cfg, mode, rows); model = load_model(cfg["model_4b"], cfg); tok = tokenizer(cfg["model_4b"]); output = []
    branches = {
        "f0": root(cfg, mode, "depth_linear_stage_a") / "best.pt",
        "ce": root(cfg, mode, "depth_linear_ce") / "best_generation.pt",
        "kd": root(cfg, mode, "depth_linear_kd") / "best_generation.pt",
    }
    for branch, checkpoint in branches.items():
        writer = writer_for(cfg, mode, "linear_continued", checkpoint)
        output += evaluate_condition(cfg, model, tok, store, rows["test"], f"depth_linear_{branch}_correct", writer)
        output += evaluate_condition(cfg, model, tok, store, rows["test"], f"depth_linear_{branch}_shuffled", writer, True)
        del writer; torch.cuda.empty_cache()
    summary = aggregate(output); floor = load_json(Path(cfg["audit_4b_dir"]) / "artifacts" / "development" / "summary.json")["conditions"]["question_only"]["f1"]
    best_branch = max((summary[f"depth_linear_{branch}_correct"]["f1"], branch) for branch in branches)[1]
    correct = summary[f"depth_linear_{best_branch}_correct"]["f1"]; shuffled = summary[f"depth_linear_{best_branch}_shuffled"]["f1"]
    decision = {"best_branch": best_branch, "correct_f1": correct, "shuffled_f1": shuffled, "question_only_f1": floor, "correct_above_no_memory": correct > floor, "correct_above_shuffled": correct > shuffled, "authorized_to_continue_to_v2": True}
    out = root(cfg, mode, "depth_linear_evaluation"); out.mkdir(parents=True, exist_ok=True); save_json(out / "per_sample.json", output); save_json(out / "summary.json", summary); save_json(out / "decision.json", decision)
    del model; torch.cuda.empty_cache()
...
def main():
    ...
    actions = {
        ...
        "depth_overfit": lambda c, m: train_stage_a(c, m, "linear_continued", True, True),
        "depth_stagea": lambda c, m: train_stage_a(c, m, "linear_continued", False, True),
        "depth_f0": depth_linear_f0,
        "depth_ce": lambda c, m: train_depth_stage_b(c, m, "ce"),
        "depth_kd": lambda c, m: train_depth_stage_b(c, m, "kd"),
        "depth_evaluate": depth_linear_evaluate,
