import torch
from torch import nn
import torch.nn.functional as F


class NativeKVTranslator(nn.Module):
    """Full28-Diagonal with a variable token dimension."""

    def __init__(self, architecture="full28_diagonal", source_layers=28, target_layers=36, heads=8, dim=128):
        super().__init__()
        if architecture != "full28_diagonal":
            raise ValueError(architecture)
        self.architecture = architecture
        self.source_layers = source_layers
        self.target_layers = target_layers
        self.heads = heads
        self.dim = dim
        width = source_layers * dim
        self.k_depth = nn.ModuleList([nn.Linear(width, dim, bias=False) for _ in range(target_layers)])
        self.v_depth = nn.ModuleList([nn.Linear(width, dim, bias=False) for _ in range(target_layers)])
        self.k_head = nn.ModuleList([
            nn.ModuleList([nn.Linear(dim, dim, bias=False) for _ in range(heads)])
            for _ in range(target_layers)
        ])
        self.v_head = nn.ModuleList([
            nn.ModuleList([nn.Linear(dim, dim, bias=False) for _ in range(heads)])
            for _ in range(target_layers)
        ])

    def _depth(self, tensor, mappings):
        x = tensor.permute(0, 2, 3, 1, 4).flatten(-2)
        return torch.stack([mapping(x) for mapping in mappings], 1)

    def _heads(self, tensor, mappings):
        return torch.stack([
            torch.stack([
                mappings[layer][head](tensor[:, layer, :, head])
                for head in range(self.heads)
            ], 2)
            for layer in range(self.target_layers)
        ], 1)

    def forward(self, key, value):
        if key.shape != value.shape or key.ndim != 5:
            raise ValueError(f"Expected matching rank-5 K/V, got {key.shape}, {value.shape}")
        if key.shape[1] != self.source_layers or tuple(key.shape[3:]) != (self.heads, self.dim):
            raise ValueError(
                f"Expected [batch,{self.source_layers},tokens,{self.heads},{self.dim}], got {tuple(key.shape)}"
            )
        if key.shape[2] < 1:
            raise ValueError("Token dimension must be non-empty")
        return (
            self._heads(self._depth(key, self.k_depth), self.k_head),
            self._heads(self._depth(value, self.v_depth), self.v_head),
        )


def component_loss(prediction, target):
    prediction, target = prediction.float(), target.float()
    nmse = (prediction - target).square().mean() / target.square().mean().clamp_min(1e-8)
    cosine = F.cosine_similarity(prediction, target, dim=-1).mean()
    return nmse + 1 - cosine, nmse, cosine


def representation_loss(pred_k, pred_v, target_k, target_v):
    kl, kn, kc = component_loss(pred_k, target_k)
    vl, vn, vc = component_loss(pred_v, target_v)
    return kl + vl, {"k_nmse": kn, "v_nmse": vn, "k_cosine": kc, "v_cosine": vc}
