...
from p3d3_common import (MemoryCache, answer_scores, forward_answer, generate, hard_negative_mapping,
                         load_receiver, seed_everything, write_json, write_jsonl)
...
def monitor(model, tokenizer, reader, cache, negatives, indices, device, max_new_tokens):
    ...
    return {"correct_em": sum(row["correct_em"] for row in rows) / len(rows),
...
def main():
    ...
    cache = MemoryCache(args.memory); total = min(args.max_samples or len(cache), len(cache)); indices = list(range(total))
    negatives_all = hard_negative_mapping(cache); negatives = [negatives_all[index] for index in indices]
    model, tokenizer = load_receiver(args.model, device)
    selected_layers = cache.index["original_layer_indices"]
    reader = NativeHeadwiseReader(model, selected_layers, rank=args.rank, gate_init=args.gate_init).to(device)
    ...
    monitor_indices = random.Random(args.seed + 77).sample(indices, min(args.monitor_samples, total))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for sample_index in tqdm(order, desc=f"p3e_a_{args.mode}_epoch{epoch}"):
            payload, wrong_payload = cache.load(sample_index), cache.load(negatives_all[sample_index])
            ...
            loss = correct_nll + args.depend_weight * dependency; loss.backward()
        ...
        record = {"epoch": epoch, "train_loss": epoch_loss, "correct_answer_mean_nll": sum(correct_values) / len(correct_values),
                  "shuffled_answer_mean_nll": sum(wrong_values) / len(wrong_values), "gates": reader.gates().detach().cpu().tolist()}
        if epoch % args.monitor_every == 0 or epoch == args.epochs:
            reader.eval(); metrics, rows = monitor(model, tokenizer, reader, cache, negatives_all, monitor_indices, device, args.max_new_tokens)
        ...
        if epoch_loss < best_loss:
            ...
            best_state = copy.deepcopy({name: tensor.detach().cpu() for name, tensor in reader.state_dict().items()})
            torch.save({"reader": best_state, "reader_metadata": reader.metadata(), "mode": args.mode, "epoch": epoch,
                        "train_loss": best_loss, "memory_index": args.memory, "args": vars(args),
                        "receiver_backbone_frozen": True, "canonical_projection_used": False}, output / "checkpoint_best.pt")
    result = {"status": "complete", "experiment": "P3-E-A receiver_native_headwise16", "mode": args.mode, "samples": total,
              "best_epoch_by_train_objective": best_epoch, "best_train_loss": best_loss, "validation_used_for_selection": False,
              "loss": "answer-token mean NLL + 0.5 * shuffled margin", "reader_metadata": reader.metadata(),
              "trainable_parameters": sum(parameter.numel() for parameter in reader.parameters()),
              "only_query_adapters_and_gates_trainable": True, "checkpoint": str(output / "checkpoint_best.pt")}
