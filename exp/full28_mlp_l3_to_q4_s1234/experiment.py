...
from data import asset_root, asset_signature, load_pair, load_source, manifest_rows
...
def receiver_memory(pair, content):
    key, value = content
    return (torch.cat((pair["target_k"][:, :1].cuda(), key), 1),
            torch.cat((pair["target_v"][:, :1].cuda(), value), 1))


def receiver_logits(model, row, key, value, suffix_start=33):
    return final_logits(model, row["encoded"]["qwen"]["answer"], key, value,
                        positions=torch.arange(key.shape[1], device="cuda"), suffix_start=suffix_start)
...
def teacher_path(cfg, split, row):
    return asset_root(cfg) / "oracle_teacher" / split / f"{row['id']}.pt"


def load_teacher(cfg, split, row):
    payload = torch.load(teacher_path(cfg, split, row), map_location="cpu", weights_only=True)
    if payload["signature"] != asset_signature(cfg): raise RuntimeError("Reused teacher mismatch")
    return payload
...
def prepare_oracle_teachers(cfg, model):
    for split in ("train", "validation", "test"):
        ...
        for number, row in enumerate(selected, 1):
            ...
            logits = receiver_logits(model, row, pair["target_k"].cuda(), pair["target_v"].cuda())
            ...
            if number % 32 == 0 or number == len(selected):
                log(f"Standard-prompt Oracle teacher {split}: {number}/{len(selected)}")
...
def evaluate_native_baselines(cfg, model):
    ...
    for number, row in enumerate(selected, 1):
        ...
        ids, gold = row["encoded"]["qwen"]["choice_ids"], row["gold_index"]
        native0_k, native0_v = pair["target_k"][:, :1].cuda(), pair["target_v"][:, :1].cuda()
        zero_k = torch.cat((native0_k, torch.zeros_like(pair["target_k"][:, 1:].cuda())), 1)
        zero_v = torch.cat((native0_v, torch.zeros_like(pair["target_v"][:, 1:].cuda())), 1)
        logits_by_name = {
            "qwen_full_native": pair["qwen_full_choice_logits"].cuda(),
            "llama_full_native": pair["llama_full_choice_logits"].cuda(),
            "qwen_self_selected32_native": receiver_logits(model, row, pair["qwen_self_k"].cuda(), pair["qwen_self_v"].cuda())[ids],
            "llama_selected_qwen_native_oracle": teacher["choice_logits"].cuda(),
            "zero": receiver_logits(model, row, zero_k, zero_v)[ids],
            "no_memory": receiver_logits(model, row, native0_k, native0_v, suffix_start=1)[ids],
        }
...
def append_results_document(cfg, comparison):
    ...
    tag = "mmlupro_standardprompt_rawanchor_full28_mlp_llama3_2_3b_to_qwen3_4b_seed1234"
    ...
    section = "\n".join([begin, f"## {tag}", "",
        "Machine-generated after successful completion. Full28 MLP (3584→1024→128), standard direct-MCQ prompt: Question → Options → Answer:.", "",
...
def finalize(cfg):
    ...
    main_table = {
        ...
        "Translated Full28-MLP": primary["translated"],
    ...
    translated_accuracy = main_table["Translated Full28-MLP"]["accuracy"]
    zero, no_memory = main_table["Zero"]["accuracy"], main_table["No-memory"]["accuracy"]
    comparison = {"experiment": "mmlupro_standardprompt_rawanchor_full28_mlp_llama3_2_3b_to_qwen3_4b_seed1234",
        "status": "completed", "prompt": "Question -> Options -> Answer:",
        "routing_query": "last Sender-native Answer: token", "selection": "32 raw-text regions",
    ...
    log("STANDARD-PROMPT FULL28-MLP EXPERIMENT COMPLETED")
    return comparison
